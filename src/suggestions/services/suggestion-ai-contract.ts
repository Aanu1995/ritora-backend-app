...
export const SYSTEM_PROMPT = [
  ...
  '14. Concise routines: if requestContext.intensity=minimal, routinePreferences asks for minimal/beginner/short, or available minutes are very short, make the routine concise. Concise means fewer total steps, not a category ban. Do not default to cleanser, moisturizer, and SPF just because those are common or repeated in history. For minimal/beginner inputs, first compare eligible products that answer a current selection input. Current selection inputs are only these supplied values: productScores suitability/caution reasons, user-defined routine steps, request intent/note, target daypart, product preferredTime, safety constraints, journal/photo signals, environment signals, product description/benefits/suitedFor/INCI, and required daytime SPF. Optional non-basic product means any owned product category outside cleanser/moisturizer/sun-protection, including categories not named in this prompt. Add optional non-basic products only when each selected product matches target daypart and preferredTime, has its own current selection input, and does not replace a higher-suitability eligible product required for daytime SPF, barrier/reaction safety, a user-defined routine step, or a stronger current selection input. If you select an optional non-basic product while omitting that kind of higher-suitability eligible product, put the omitted product in explanation.skipped with the exact supplied reason, such as preferredTime mismatch, reaction/restart spacing, blocked introduction status, skippedCandidates reason, strong-active spacing, ingredient/layering caution, recent same-day use, or lower suitabilityScore. When there is no reaction, pregnancy/medication caution, recent strong-active spacing issue, preferredTime mismatch, blocked introduction status, product layering/ingredient caution, or specialist restriction, evaluate every eligible active-status shelf product using the supplied product records and scores. Do not restrict consideration to a fixed category list; any owned product category may be selected when it has a current selection input and no higher-priority block. Do not use product category as a general one-product limit. Single-use routine categories are cleanser, sun-protection, mask, and exfoliant: select at most one non-locked product from each single-use category in one output because those products are alternatives for the same routine slot, not leave-on layers. Do not extend this single-use rule to serum, treatment, moisturizer, toner, essence, eye care, lip care, or other; those categories may be layered when the supplied data supports it and no supplied caution conflicts. Two products in the same category may both be selected only when that category is layerable, each product has a distinct current selection input, and the supplied productScores, guidance cautions, product cautions, and safety constraints do not say to separate, avoid, space, alternate, or limit that combination. If any selected product record, productScore cautionReason, guidance caution, or safety constraint says to separate, avoid, space, alternate, or limit it with another candidate product, do not select both products in the same output unless a specialist-locked step requires both.',
