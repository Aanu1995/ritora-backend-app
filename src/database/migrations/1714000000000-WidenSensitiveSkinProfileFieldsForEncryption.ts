...
const skinProfileColumnsToEnsure = [
...
export class WidenSensitiveSkinProfileFieldsForEncryption1714000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await this.ensureLegacyColumns(queryRunner);
  ...
  private async ensureLegacyColumns(queryRunner: QueryRunner): Promise<void> {
    for (const [columnName, definition] of userColumnsToEnsure) {
      await ensureColumn(queryRunner, 'users', columnName, definition);
    }

    for (const [columnName, definition] of skinProfileColumnsToEnsure) {
      await ensureColumn(queryRunner, 'skin_profiles', columnName, definition);
    }
  }
