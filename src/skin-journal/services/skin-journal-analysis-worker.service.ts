...
import {
  SKIN_JOURNAL_ANALYSIS_JOB_POLL_INTERVAL_MS,
  SKIN_JOURNAL_ANALYSIS_SQS_VISIBILITY_HEARTBEAT_MS,
} from '../skin-journal.constants';
import {
  AnalysisQueueMessage,
  SkinJournalAnalysisQueueService,
} from './skin-journal-analysis-queue.service';

@Injectable()
export class SkinJournalAnalysisWorkerService
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(SkinJournalAnalysisWorkerService.name);
  private readonly workerId = `skin-journal-analysis-${ulid()}`;
  ...
  constructor(
    private readonly queue: SkinJournalAnalysisQueueService,
  ...
  async pollOnce(): Promise<void> {
    ...
    try {
      ...
      if (job) {
        await this.journal.processAnalysisJob(job);
      }
    } finally {
      this.polling = false;
    }
  ...
  private async processSqsMessage(
    ...
    try {
      ...
      this.logger.error(
        `Failed to process skin journal analysis job ${message.jobId}`,
        error,
      );
    } finally {
      if (heartbeat) {
  ...
  private startVisibilityHeartbeat(
    ...
    const heartbeat = setInterval(() => {
      void this.queue.extendMessageVisibility(receiptHandle).catch((error) => {
        this.logger.warn(
          `Failed to extend Skin Journal analysis message visibility: ${
  ...
  private schedulePoll(delayMs: number): void {
    ...
    this.pollTimer = setTimeout(() => {
      void this.pollOnce()
        .catch((error) => {
          this.logger.error('Skin journal analysis worker poll failed', error);
        })
        .finally(() => {
          if (this.enabled && !this.stopped) {
            this.schedulePoll(SKIN_JOURNAL_ANALYSIS_JOB_POLL_INTERVAL_MS);
          }
        });
    }, delayMs);
